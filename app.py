...
def find_best_dse_subject_col(columns, subject_keywords, exclude_keywords=None):
    ...
    all_excludes = set(
        [k.upper() for k in (exclude_keywords + default_excludes)]
    )
...
def get_school_subject_score(df, subject_keys, exclude_keys):
    ...
    for col in matched_cols:
        ...
        combined_series = (
            s if combined_series is None else combined_series.fillna(s)
        )
...
def compute_all_subjects_correlation(d_sch, d_dse):
    ...
    dse_reg_cols = [
        c
        for c in d_dse.columns
        if "Registration No" in str(c)
        or "Reg" in str(c)
        or "註冊編號" in str(c)
        or "學號" in str(c)
    ]
    ...
    for sub in SUBJECT_MAP:
        sch_s = get_school_subject_score(
            m_pair, sub["sch_keys"], sub["sch_ex"]
        )
    ...
    if not df_res.empty:
        df_res = df_res.sort_values(
            by="校內分數 vs DSE 等級 相關係數 (r)", ascending=False
        )
    return df_res
...
def extract_robust_scores(df):
    ...
    avg_score = (
        pd.to_numeric(df_out[tot_col], errors="coerce").round(1)
        if tot_col
        else pd.Series(np.nan, index=df_out.index)
    )

    chi_series = get_school_subject_score(
        df_out, ["中文"], ["閱讀", "寫作", "聆聽", "口試", "說話", "中史"]
    )
    eng_series = get_school_subject_score(
        df_out, ["英文"], ["閱讀", "作文", "聆聽", "口試", "語文"]
    )
    math_series = get_school_subject_score(
        df_out,
        ["數必", "數學"],
        ["數一", "數二", "M1", "M2", "EXTENDED", "單元"],
    )

    cs_cols = [
        c
        for c in df_out.columns
        if any(k in str(c) for k in ["公民科", "CS"])
        and any(k in str(c) for k in ["Score", "Grade"])
    ]
    cs_series = None
    for csc in cs_cols:
    ...
    df_out["Chi_Score"] = (
        chi_series.round(1) if chi_series is not None else np.nan
    )
    df_out["Eng_Score"] = (
        eng_series.round(1) if eng_series is not None else np.nan
    )
    df_out["Math_Score"] = (
        math_series.round(1) if math_series is not None else np.nan
    )
...
def process_training_pair(f_sch, f_dse, dataset_label="2526"):
    ...
    if len(m_ai) == 0:
        return (
            ...
            pd.DataFrame(),
            (
                f"⚠️ 對照失敗：無法透過校內【{sch_reg}】與"
                f" DSE【{dse_reg}】匹配學生身份，請確認兩檔學號是否相符。"
            ),
        )
    ...
    math_col = find_best_dse_subject_col(
        d_dse.columns,
        ["A030", "MATH", "數學"],
        ["M1", "M2", "數一", "數二", "EXTENDED", "單元"],
    )
    ...
    clean_df = m_ai.dropna(subset=feats + ["Target_332"])[
        feats
        + ["Target_332", "Target_222", "DSE_Chi_Lvl", "DSE_Eng_Lvl", "DSE_Math_Lvl"]
    ]
    ...
    for sub in SUBJECT_MAP[3:]:  # 選修科目
        sub_name = sub["name"]
        sch_s = get_school_subject_score(
            comp_df, sub["sch_keys"], sub["sch_ex"]
        )
...
if "u_score_val" not in st.session_state:
    st.session_state["u_score_val"] = 50.4
if "u_chi_val" not in st.session_state:
    st.session_state["u_chi_val"] = 49.8
if "u_eng_val" not in st.session_state:
    st.session_state["u_eng_val"] = 53.8
if "u_math_val" not in st.session_state:
    st.session_state["u_math_val"] = 38.8
if "u_cs_val" not in st.session_state:
    st.session_state["u_cs_val"] = 40.0

# 側邊欄門檻設定（為每個元件設定唯一的 key 以避免重複 ID 錯誤）
st.sidebar.header("⚙️ 大學門檻 (332A22) 校內分數設定")

u_score_thresh = st.sidebar.number_input(
    "大學：總平均分門檻",
    value=float(st.session_state["u_score_val"]),
    key="u_score_thresh_input",
)
u_chi_thresh = st.sidebar.number_input(
    "大學：中文分數門檻",
    value=float(st.session_state["u_chi_val"]),
    key="u_chi_thresh_input",
)
u_eng_thresh = st.sidebar.number_input(
    "大學：英文分數門檻",
    value=float(st.session_state["u_eng_val"]),
    key="u_eng_thresh_input",
)
u_math_thresh = st.sidebar.number_input(
    "大學：數學分數門檻",
    value=float(st.session_state["u_math_val"]),
    key="u_math_thresh_input",
)
u_cs_thresh = st.sidebar.number_input(
    "大學：公社科分數門檻",
    value=float(st.session_state["u_cs_val"]),
    key="u_cs_thresh_input",
)

st.sidebar.header("⚙️ 大專門檻 (222A22) 校內分數設定")
sub_score_thresh = st.sidebar.number_input(
    "大專：總平均分門檻", value=40.0, key="sub_score_thresh_input"
)
sub_chi_thresh = st.sidebar.number_input(
    "大專：中文分數門檻", value=40.0, key="sub_chi_thresh_input"
)
sub_eng_thresh = st.sidebar.number_input(
    "大專：英文分數門檻", value=40.0, key="sub_eng_thresh_input"
)
sub_math_thresh = st.sidebar.number_input(
    "大專：數學分數門檻", value=40.0, key="sub_math_thresh_input"
)
...
with main_tab1:
    ...
    st.info(
        "系統會優先自動偵測專案目錄中的數據檔案（如 `2526_T1A3_s6.xlsx`,"
        " `2526hkdse.xlsx`, `2425_T1A3_s6.xlsx`,"
        " `2425hkdse.xlsx`）。若未找到，亦可透過下方按鈕手動上載。"
    )

    st.markdown("##### 📁 第一套歷史數據 (例如 2526 學年畢業生)")
    col_a1, col_b1 = st.columns(2)
    with col_a1:
        f_tr_school_1_up = st.file_uploader(
            "1A. 上傳/替換【2526】校內模擬試成績",
            type=["xls", "xlsx", "csv"],
            key="tr_s_1",
        )
        f_tr_school_1, msg_s1 = resolve_file_source(
            f_tr_school_1_up, ["2526_T1A3_s6.xlsx", "2526_T1A3_s6.csv"]
        )
        if f_tr_school_1:
            st.caption(msg_s1)
        else:
            st.caption("⚠️ 未找到 2526 校內成績預設檔，請手動上載。")

    with col_b1:
        f_tr_dse_1_up = st.file_uploader(
            "1B. 上傳/替換【2526】2526HKDSE 公開試成績",
            type=["xls", "xlsx", "csv"],
            key="tr_d_1",
        )
        f_tr_dse_1, msg_d1 = resolve_file_source(
            f_tr_dse_1_up, ["2526hkdse.xlsx", "2526hkdse.csv"]
        )
        if f_tr_dse_1:
            st.caption(msg_d1)
        else:
            st.caption("⚠️ 未找到 2526HKDSE 預設檔，請手動上載。")
    ...
    if f_tr_school_1 and f_tr_dse_1:
        df1, corr_df1, comp1, err1 = process_training_pair(
            f_tr_school_1, f_tr_dse_1, "2526"
        )
        if err1:
            ...
            if corr_df1 is not None and not corr_df1.empty:
                corr_dfs.append(("2526", corr_df1))
            if comp1 is not None and not comp1.empty:
                comp_dfs.append(comp1)

    st.markdown(
        "##### 📁 第二套歷史數據 (例如 2425 學年畢業生 - 增加樣本量與精準度)"
    )
    col_a2, col_b2 = st.columns(2)
    with col_a2:
        f_tr_school_2_up = st.file_uploader(
            "2A. (選填) 上傳/替換【2425】校內模擬試成績",
            type=["xls", "xlsx", "csv"],
            key="tr_s_2",
        )
        f_tr_school_2, msg_s2 = resolve_file_source(
            f_tr_school_2_up, ["2425_T1A3_s6.xlsx", "2425_T1A3_s6.csv"]
        )
        if f_tr_school_2:
            st.caption(msg_s2)
        else:
            st.caption("⚠️ 未找到 2425 校內成績預設檔，可選擇手動上載。")

    with col_b2:
        f_tr_dse_2_up = st.file_uploader(
            "2B. (選填) 上傳/替換【2425】2425HKDSE 公開試成績",
            type=["xls", "xlsx", "csv"],
            key="tr_d_2",
        )
        f_tr_dse_2, msg_d2 = resolve_file_source(
            f_tr_dse_2_up, ["2425hkdse.xlsx", "2425hkdse.csv"]
        )
        if f_tr_dse_2:
            st.caption(msg_d2)
        else:
            st.caption("⚠️ 未找到 2425HKDSE 預設檔，可選擇手動上載。")

    # 處理第二套
    if f_tr_school_2 and f_tr_dse_2:
        df2, corr_df2, comp2, err2 = process_training_pair(
            f_tr_school_2, f_tr_dse_2, "2425"
        )
        if err2:
            ...
            if corr_df2 is not None and not corr_df2.empty:
                corr_dfs.append(("2425", corr_df2))
            if comp2 is not None and not comp2.empty:
                comp_dfs.append(comp2)
    ...
    if train_dfs and HAS_SKLEARN:
        try:
            ...
            st.success(
                f"🎉 AI 模型訓練成功！已合併 {len(train_dfs)}"
                f" 套歷史數據（總訓練樣本數：{len(df_train)}"
                " 人），並提煉 332 達標學生下四分位數保底門檻 (Q1 基準)。"
            )
            ...
            with col_tree1:
                ...
                imp_df = pd.DataFrame({
                    "校內指標": feats,
                    "AI 預測權重": clf.feature_importances_,
                }).sort_values("AI 預測權重", ascending=False)
                st.dataframe(imp_df, use_container_width=True, hide_index=True)

            with col_tree2:
                st.markdown(
                    "##### 🌲 AI 自動提煉的數據決策樹規則 (Exact Cutoffs)："
                )
    ...
    if comp_dfs:
        st.divider()
        st.subheader(
            "📊 達大學 (332) 或大專 (222) 收生門檻之『公開試成績 vs"
            " 校內成績』對照表與平均分總覽"
        )
        ...
        sub_group = df_all_comp[
            df_all_comp["達標類別"].str.contains("222")
        ]

        u_avg_val = (
            safe_mean(u_group["校內總平均分"])
            if "校內總平均分" in u_group
            else np.nan
        )
        sub_avg_val = (
            safe_mean(sub_group["校內總平均分"])
            if "校內總平均分" in sub_group
            else np.nan
        )

        c1, c2, c3, c4 = st.columns(4)
        with c1:
            st.metric("🎓 達大學門檻 (332) 總人數", f"{len(u_group)} 人")
        with c2:
            st.metric(
                "🎓 達大學門檻學生【校內總平均分】",
                (
                    f"{u_avg_val:.1f} 分"
                    if not pd.isna(u_avg_val)
                    else "N/A"
                ),
            )
        with c3:
            st.metric("🏫 達大專門檻 (222) 總人數", f"{len(sub_group)} 人")
        with c4:
            st.metric(
                "🏫 達大專門檻學生【校內總平均分】",
                (
                    f"{sub_avg_val:.1f} 分"
                    if not pd.isna(sub_avg_val)
                    else "N/A"
                ),
            )
        ...
        st.markdown(
            "##### 📋 達標學生『公開試等級 vs 校內成績』明細表"
        )

        # 雙條件篩選列：1. 達標類別  2. 學年
        col_f1, col_f2 = st.columns([1, 1])
        with col_f1:
            filter_tier = st.radio(
                "篩選達標類別：",
                ["全部達標學生", "🎓 僅大學門檻 (332)", "🏫 僅大專門檻 (222)"],
                horizontal=True,
                key="filter_tier_radio",
            )

        with col_f2:
            available_years = ["全部學年"] + sorted(
                list(df_all_comp["學年"].astype(str).unique()), reverse=True
            )
            filter_year = st.selectbox(
                "篩選學年：", available_years, key="filter_year_select"
            )
        ...
        if filter_tier == "🎓 僅大學門檻 (332)":
            df_disp_filtered = df_disp_filtered[
                df_disp_filtered["達標類別"].str.contains("332")
            ]
        elif filter_tier == "🏫 僅大專門檻 (222)":
            df_disp_filtered = df_disp_filtered[
                df_disp_filtered["達標類別"].str.contains("222")
            ]
    ...
    if corr_dfs:
        with st.expander(
            ...
            for label, c_df in corr_dfs:
                ...
                st.dataframe(
                    c_df, use_container_width=True, hide_index=True
                )
...
with main_tab2:
    st.subheader(
        "🔮"
        " 校內成績預測：產生「大學」、「特別支援（差一科）」、「大專」及「保底」名單"
    )
    f_eval_up = st.file_uploader(
        "上傳/替換校內成績表 (Excel / CSV)",
        type=["xls", "xlsx", "csv"],
        key="eval_up",
    )
    f_eval, msg_eval_tab3 = resolve_file_source(
        f_eval_up, ["2526_T1A3_s6.xlsx", "2526_T1A3_s6.csv"]
    )
    if f_eval:
        st.caption(msg_eval_tab3)
    ...
    if f_eval:
        try:
            ...
            for sub in SUBJECT_MAP[3:]:  # 選修科配置
                s = get_school_subject_score(
                    df_ev, sub["sch_keys"], sub["sch_ex"]
                )
            ...
            def cs_is_attained(cs_val):
                ...
                if val_str in ["U", "UNATTAINED", "不達標", "FAIL", "F"]:
                    return False
                try:
                    return float(val_str) >= u_cs_thresh
                except Exception:
                    return False
            ...
            df_ev["公民與社會發展科"] = df_ev["CS_Val"].apply(
                format_cs_score
            )
            ...
            def categorize_student(row):
                ...
                failed_conds = [
                    cond_names[i]
                    for i, passed in enumerate(cond_u)
                    if not passed
                ]
                ...
                if all(cond_u):
                    ...
                    and pd.notna(e)
                    and pd.notna(m)
                    and s >= sub_score_thresh
                    and c >= sub_chi_thresh
                    and e >= sub_eng_thresh
                    and m >= sub_math_thresh
                    and cs_ok
                ):
                    return pd.Series([
                        "🏫 潛質入大專名單 (222A22)",
                    ...
                    return pd.Series([
                        "🛟 保底求合格名單 (關鍵科需支援)",
                        f"未達標科目：{', '.join(failed_conds)}",
                    ])

            df_ev[["升學類別", "診斷與提示"]] = df_ev.apply(
                categorize_student, axis=1
            )
            ...
            with m1:
                st.metric(
                    "🎓 潛質入大學 (332A22)",
                    f"{counts.get('🎓 潛質入大學名單 (332A22)', 0)} 人",
                )
            with m2:
                st.metric(
                    "🎯 特別支援 (差一科)",
                    f"{counts.get('🎯 特別支援名單 (差一科入大學)', 0)} 人",
                )
            with m3:
                st.metric(
                    "🏫 潛質入大專 (222A22)",
                    f"{counts.get('🏫 潛質入大專名單 (222A22)', 0)} 人",
                )
            with m4:
                st.metric(
                    "🛟 保底求合格名單",
                    f"{counts.get('🛟 保底求合格名單 (關鍵科需支援)', 0)} 人",
                )
            ...
            if "*Class" in df_ev.columns:
                disp_ev.append("*Class")
            if "*Class Number" in df_ev.columns:
                disp_ev.append("*Class Number")
            if "中文姓名" in df_ev.columns:
                disp_ev.append("中文姓名")
            if "*Student Name" in df_ev.columns:
                disp_ev.append("*Student Name")
            ...
            sort_cols = [
                c
                for c in ["*Class", "*Class Number"]
                if c in df_ev.columns
            ]

            with tab_u:
                df_sub_u = df_ev[df_ev["升學類別"].str.contains("大學名單")][
                    disp_ev
                ]
                if sort_cols:
                    df_sub_u = df_sub_u.sort_values(sort_cols)
                st.dataframe(
                    df_sub_u, use_container_width=True, hide_index=True
                )
            with tab_sp:
                df_sub_sp = df_ev[df_ev["升學類別"].str.contains("特別支援")][
                    disp_ev
                ]
                if sort_cols:
                    df_sub_sp = df_sub_sp.sort_values(sort_cols)
                st.dataframe(
                    df_sub_sp, use_container_width=True, hide_index=True
                )
            with tab_sub:
                df_sub_sub = df_ev[df_ev["升學類別"].str.contains("大專名單")][
                    disp_ev
                ]
                if sort_cols:
                    df_sub_sub = df_sub_sub.sort_values(sort_cols)
                st.dataframe(
                    df_sub_sub, use_container_width=True, hide_index=True
                )
            with tab_base:
                df_sub_base = df_ev[df_ev["升學類別"].str.contains("保底求合格")][
                    disp_ev
                ]
                if sort_cols:
                    df_sub_base = df_sub_base.sort_values(sort_cols)
                st.dataframe(
                    df_sub_base, use_container_width=True, hide_index=True
                )
